...
getDataFromUrlMethod.prototype.execute = function(params) {
    ...
    if(valid){
        this.server.HttpProvider.download(params.host, params.path, function(err, res) {
            if (err) {
                that._dispatchError(params, -31990, 'Http error!');
            } else {
                try{
                    ...
                    that._dispatchError(params, -31991, 'Invalid Json');
                }
            }
        });
    }
    else{
        that._dispatchError(params, -31992, 'Parameters doesn\'t match the requirements');
    }
};
