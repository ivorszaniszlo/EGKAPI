...
getProductMethod.prototype.requiredParams = ['host', 'path', 'productId'];
//Parse the url in product.options
//const product = jayson.client.http(host + '/' + path + q + cid);
//get URL from Object
//var url = product.options.href;

getProductMethod.prototype.paramsConverter = function(params){
    var params = 
    {
        "host": host,
        "path": path,
       // "url": product.options.href,
        "productId": productId
    };
    ...
    for(var i in params){
        ...
        else if(i=='productId' && params[i][0]!='/'){
            returnData[i]='/'+params[i] + q+ cid;
        }
...
getProductMethod.prototype.execute = function(params) {
    ...
    console.info('Executing getProductMethod, HOST:', host,'PATH:', path + cid, 'URL:', url);
    var valid = false;
    ...
    if(valid){
        this.server.httpProvider.download(params.host, params.path, params.url, params.productId, function(err, res) {
            if (err) {
